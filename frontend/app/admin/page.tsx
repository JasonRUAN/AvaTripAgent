...
import { type AgentHealth, isHttpUrl, probeAgentHealth } from "@/lib/agent-health";
...
export default function AdminPage() {
  ...
  const onProbe = async () => {
    ...
    const result = await probeAgentHealth(endpoint);
    setHealth(result);
  };
  ...
  const onRegister = async () => {
    ...
    if (!isAddress(agent) || !name.trim() || !endpoint.trim()) {
      setMessage("请填写有效地址、名称和 HTTP endpoint");
      return;
    }
    if (categories.length === 0) {
      setMessage("至少选择一个服务分类");
      return;
    }
    if (website.trim() && !isHttpUrl(website)) {
      setMessage("官网需以 http:// 或 https:// 开头");
      return;
    }
    if (!isHttpUrl(endpoint)) {
      setMessage("API 地址需以 http:// 或 https:// 开头");
      return;
    }
    try {
      await register({
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6">
      <div className="mb-6">
        <h1 className="font-display text-2xl font-bold tracking-tight text-brand-gradient">服务商管理</h1>
      ...
      {!isConnected ? (
        <div className="mt-10 flex flex-col items-center gap-3 text-center">
          <p className="text-sm text-ink-500">请先连接管理员钱包</p>
          <WalletButton />
        </div>
      ) : null}
      ...
      {isOwner ? (
        <div ref={formRef} className="mb-8 flex flex-col gap-4 rounded-3xl border border-brand-100 bg-white/90 p-5 shadow-card">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm font-bold text-ink-800">{editing ? "更新 Agent" : "注册 Agent"}</p>
            {editing ? (
              <button
                type="button"
                onClick={() => {
          ...
          <label className="flex flex-col gap-1.5">
            <span className="text-[11px] font-bold tracking-wide text-ink-400">Agent 地址</span>
            <input
          ...
          <label className="flex flex-col gap-1.5">
            <span className="text-[11px] font-bold tracking-wide text-ink-400">名称</span>
            <input className={inputClass} placeholder="AvaFlights Agent" value={name} onChange={(e) => setName(e.target.value)} />
          </label>

          <div>
            <p className="mb-2 text-[11px] font-bold tracking-wide text-ink-400">服务分类（至少选一，可多选）</p>
            ...
            {categories.length === 0 ? (
              <p className="mt-1.5 text-[11px] text-coral-500">请至少选择一个分类</p>
            ) : null}
          </div>
          ...
          <label className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              placeholder="一句话介绍这家服务商能为旅行者做什么"
              value={description}
          ...
          <label className="flex flex-col gap-1.5">
            <span className="text-[11px] font-bold tracking-wide text-ink-400">官网链接</span>
            <input
              className={inputClass}
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-[11px] font-bold tracking-wide text-ink-400">服务商 API 访问地址</span>
            ...
            {health.status === "ok" ? (
              <p className="text-[11px] text-ink-400">
                {health.address ? `${health.address.slice(0, 10)}…` : ""}
          ...
          >
            {isPending ? "提交中…" : editing ? "更新 registerAgent" : "registerAgent"}
          </button>
      ...
      <section className="rounded-3xl border border-brand-100 bg-white/70 p-4 shadow-card sm:p-5">
        <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
          <Stat label="服务商" value={stats.total} />
          <Stat label="启用中" value={stats.active} />
          {stats.byCat.map((item) => (
            <Stat
              key={item.code}
              label={`${CATEGORY_ICON[item.code]} ${CATEGORY_LABEL[item.code]}`}
              value={item.count}
            />
          ))}
        </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {(["all", "active", "inactive"] as const).map((status) => (
              <FilterChip key={status} active={filterStatus === status} onClick={() => setFilterStatus(status)}>
                {status === "all" ? "全部状态" : status === "active" ? "启用" : "停用"}
              </FilterChip>
            ))}
            <input
        ...
        {listed.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-brand-200 bg-white/60 px-6 py-14 text-center">
            <p className="text-sm font-semibold text-ink-500">没有符合条件的服务商</p>
            <p className="mt-1 text-xs text-ink-400">调整筛选，或用上方表单注册一家新的 Agent</p>
          </div>
        ) : (
