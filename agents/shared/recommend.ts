/**
 * 询价后的 offer 推荐。规则需与 `frontend/lib/recommend.ts` 保持一致。
 */
...
export function scoresFromProfiles(
  profiles: Array<{
    address: Address;
    ratingAvg: number;
    ratingCount: number;
  ...
  for (const profile of profiles) {
    map[profile.address.toLowerCase()] = {
      ratingAvg: profile.ratingAvg,
      ratingCount: profile.ratingCount,
      ratingAvgX100: profile.ratingAvgX100,
      name: profile.name,
    };
  }
  return map;
...
function sortList<T extends FlightOffer | HotelOffer | AttractionOffer | DiningOffer>(
  ...
  return [...items].sort((a, b) => {
    ...
    if (mode === "value") {
      return (
        valueScore(sb.ratingAvg || bayesScore(sb.ratingAvg, sb.ratingCount), priceOfItem(b)) -
...
function pickBudgetIds(priced: OfferBundle, request: TripRequest, scores: ProviderScoreMap): string[] {
  ...
  const better = (next: { provider: Address }, prev?: { provider: Address }) => {
    if (!prev) return true;
    return (
      bayesScore(scoreOf(next.provider, scores).ratingAvg, scoreOf(next.provider, scores).ratingCount) >
