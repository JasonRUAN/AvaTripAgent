import { env, hasLLM, type AgentKey } from "../shared/env";
import { chatJson } from "../shared/llm";
import { compressOffers } from "../shared/inventory";
import {
  buildDay,
  buildQuote,
  dayDate,
...
import type { Address, ItineraryDay, StreamEvent, TripRequest } from "../shared/types";
import {
  DAY_SYSTEM,
  OUTLINE_SYSTEM,
  PARSE_SYSTEM,
...
export async function runPlan(params: {
  ...
  tripId: string;
  request: TripRequest;
  providers?: Partial<Record<AgentKey, Address>>;
  emit: Emit;
}): Promise<PlanResult> {
  ...
  // ② 询价 ----------------------------------------------------------------
  emit({ type: "agent.status", phase: "sourcing", label: "正在选择服务商并向其询价…" });
  ...
  emit({
    ...
    args: {
      ...
      pax: request.pax,
      agent: selected.flight.name,
      score: selected.flight.ratingAvg,
    },
  });
  emit({
    type: "tool.call",
    name: "search_hotels",
    args: { city: request.destination, agent: selected.hotel.name, score: selected.hotel.ratingAvg },
  });
  emit({
    type: "tool.call",
    name: "search_attractions",
    args: { city: request.destination, agent: selected.attraction.name, score: selected.attraction.ratingAvg },
  });
  emit({
    type: "tool.call",
    name: "search_restaurants",
    args: { city: request.destination, agent: selected.dining.name, score: selected.dining.ratingAvg },
  });
  ...
  emit({
    type: "tool.result",
    name: "search_flights",
    data: { count: sourced.flights.length, agents: sourced.queried.flight },
    ms: sourced.timings.flight,
  });
  emit({ type: "offer.flight", items: sourced.flights });
  emit({
    type: "tool.result",
    name: "search_hotels",
    data: { count: sourced.hotels.length, agents: sourced.queried.hotel },
    ms: sourced.timings.hotel,
  });
  emit({ type: "offer.hotel", items: sourced.hotels });
  emit({
    type: "tool.result",
    name: "search_attractions",
    data: { count: sourced.attractions.length, agents: sourced.queried.attraction },
    ms: sourced.timings.attraction,
  });
  emit({ type: "offer.attraction", items: sourced.attractions });
  emit({
    type: "tool.result",
    name: "search_restaurants",
    data: { count: sourced.restaurants.length, agents: sourced.queried.dining },
    ms: sourced.timings.dining,
  });
  emit({ type: "offer.dining", items: sourced.restaurants });
  ...
  const { quote, budgetItems } = buildQuote({ runId, tripId, request, offers, days, providerNames: names });
