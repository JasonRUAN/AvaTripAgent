...
1. **凭证用 ERC-721 还是 SBT（不可转让）？** 建议 ERC-721（可转让 = 转赠/改签叙事更强，演示更灵活）。
2. **分账是一笔原子交易还是 N 笔？** 建议 **N 笔**（视觉效果好、叙事清楚），gas 由后端承担。
3. **`settle` 由后端自动触发还是用户点确认？** 建议支付后自动触发（少一次签名），UI 用时间线展示进度。
4. **是否接真实天气/汇率 API？** 建议**不接**（保持零外部依赖，现场稳定）。
5. **tokenURI 用后端 HTTP 还是纯 on-chain？** 建议后端 HTTP（快、可改），链上只存 `metadataHash`。
